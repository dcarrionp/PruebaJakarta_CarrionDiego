package ec.edu.ups.ppw.pruebajee.PruebaJakarta_CarrionDiego.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/PruebaJakarta_CarrionDiego.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "PruebaJakarta_CarrionDiego.war").importFrom(f).as(WebArchive.class);

        // Add the package containing the test classes:
        war.addPackage("ec.edu.ups.ppw.pruebajee.PruebaJakarta_CarrionDiego.test");
